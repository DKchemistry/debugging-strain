...
def custom_sdmolsupplier_H(file_path):
    ...
    suppl_H = Chem.SDMolSupplier(
        file_path, removeHs=False
    )  # sanitizize=False causes error
    names = []
    mol = {}
...
# Global list to store 'tp' variables
match_debug_list = []
...
def tp_match(tp, hc, j, mol, pos, bi):
    ...
    for match in matches:
        ...
        match_debug_list.append(match)  # Append match to the global list
        if tp.get("method") == "exact":
